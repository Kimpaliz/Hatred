/* [Aufgabe: Prüfwesen] Malt echte Granitpixel und prüft Hexbesitz und Treppennähte.
   ...
   gehen über `werkzeuge/helfer.mjs` in die vollständige Prüfkette. */
...
abschnitt("Hexbesitz: kein Loch, kein doppelter Weltpixel");
{
  ...
  for (let y = 4; y <= 9; y++) for (let x = 4; x <= 9; x++) {
    ...
    for (let py = 0; py < bild.hoehe; py++) for (let px = 0; px < bild.breite; px++) {
      if (!bild.pixel[py * bild.breite + px]) continue;
      const wx = bild.x0 + px, wy = bild.y0 + py, key = `${wx},${wy}`;
      const f = weltNachFeld(wx + 0.5, wy + 0.5);
  ...
  for (let wy = 80; wy <= 112; wy++) for (let wx = 96; wx <= 128; wx++) {
    behaupte(besitzer.has(`${wx},${wy}`), `Innenfläche enthält Weltpixel ${wx},${wy}`);
  }
}
...
for (const y of [6, 7]) for (const r of richtungen(y)) {
  ...
  let offen = 0;
  for (let py = 0; py < ziel.hoehe; py++) for (let px = 0; px < ziel.breite; px++) {
    const wx = ziel.x0 + px + 0.5, wy = ziel.y0 + py + 0.5;
    if (Math.hypot(wx - (a.x + b.x) / 2, wy - (a.y + b.y) / 2) > 2) continue;
    if (!ziel.pixel[py * ziel.breite + px]) continue;
    const rolle = ziel.rollen[py * ziel.breite + px];
...
abschnitt("Breite und lange Treppen haben keine innere Abschlusskante");
{
  ...
  for (const [x, y, nx, ny] of paare) {
    ...
    for (const [fx, fy] of [[x, y], [nx, ny]]) {
      ...
      for (let py = 0; py < bild.hoehe; py++) for (let px = 0; px < bild.breite; px++) {
        const wx = bild.x0 + px + 0.5, wy = bild.y0 + py + 0.5;
        if (Math.hypot(wx - (a.x + b.x) / 2, wy - (a.y + b.y) / 2) > 2) continue;
        const wert = bild.pixel[py * bild.breite + px];
        if (!wert) continue;
        gleich(bild.rollen[py * bild.breite + px], 5,
          "Echte Treppennaht hat beidseitig keine Zwischenwange oder Querbarriere");
        if (((bild.x0 + px) % 4) === 0) continue;
...
abschnitt("Felsinnere ohne wiederkehrende Südflanken");
{
  ...
  for (let y = 0; y < bild.hoehe; y++) {
    const row = [...bild.pixel.slice(y * bild.breite, (y + 1) * bild.breite)].filter(Boolean);
...
for (const y of [6, 7]) for (const gleicheRichtung of [false, true]) {
  ...
  for (const [fx, fy, oben] of [[x, y, false], [nx, ny, true]]) {
    ...
    for (let py = 0; py < bild.hoehe; py++) for (let px = 0; px < bild.breite; px++) {
      const wx = bild.x0 + px + 0.5, wy = bild.y0 + py + 0.5;
      if (Math.hypot(wx - (a.x + b.x) / 2, wy - (a.y + b.y) / 2) > 2) continue;
      const rolle = bild.rollen[py * bild.breite + px];
...
abschnitt("Schräge Trittphasen bleiben unabhängig von der Zeilenlage");
{
  ...
  for (const verschiebung of [0, 10000000]) for (const richtung of [1, 2, 4, 5]) {
    ...
    for (let py = 0; py < bild.hoehe; py++) for (let px = 0; px < bild.breite; px++) {
      const wert = bild.pixel[py * bild.breite + px];
      if (!wert) continue;
      const proj = (bild.x0 + px + 0.5) * achsen[richtung][0]
        + (bild.y0 + py + 0.5) * achsen[richtung][1];
...
abschnitt("Browserpfad: ein wiederverwendetes Pixelbild statt Einzelrechtecken");
{
  ...
  const g = macheGranitFeld({ ton, ctx, kamera: { vergroesserung: 3 },
    kasten() { ersatz++; } });
  ...
  tiefGleich(zeichnungen[0].slice(1), [5, 9, feld.breite * 3, feld.hoehe * 3],
