# Dashboard Golden Path Game 2.2.0
...
`packages/dashboard-games-sdk`, `GAME_PROJECT_CONTEXT.md` als Plattformvertrag,
`AGENTS.md` als verbindliche Bauregel, das JSON-Schema und gültige
Platzhalterbilder unter `public/dashboard/`.

## Neuer Spielstand in sechs Schritten
...
4. `npm ci`, `npm test`, `npm run build` und `npm run dashboard:verify`
   ...
   `AGENTS.md` bleibt dabei unverändert liegen und gilt als Arbeitsanweisung
   für jeden Menschen und jeden Agenten in diesem Repository.
5. Vor der Anbindung `npm run check:release` ausführen und die Freigabeliste
   aus `AGENTS.md` Abschnitt 5 vollständig abarbeiten.
6. Commit pushen, den Job `verify` in den Branch-Schutzregeln des
...
## Bauregeln und Freigabe

`AGENTS.md` fasst zusammen, wie in diesem Repository gearbeitet wird: Rahmen,
harte Sicherheitsregeln, Ablauf je Pull Request und die vollständige
Freigabeliste für den ersten Web-Deploy. Sie hält außerdem fest, was
ausdrücklich **nicht** verlangt ist — keine Alpha-, Beta- oder Playtest-Phasen,
keine Lizenz-, Rechts- oder Storetexte. Diese Spiele sind private Projekte und
werden nur mit direkt eingeladenen Freunden gespielt.

Der maschinell prüfbare Teil der Freigabeliste läuft als eigener Befehl:

```text
npm run check:release
```

Er meldet je Punkt `OK` oder `FEHLT` und nennt die konkrete Datei: eigene
Manifest-Identität, eigenes Artwork in Cover-, Hero- und Icon-Budget, Version
ab `1.0.0` mit Changelog-Abschnitt, Menüstruktur und sichtbare Version im
Client, vollständiger Runtimevertrag und startbarer Workflow. Auf der
unveränderten Vorlage meldet er absichtlich offene Punkte; das ist der
Arbeitsvorrat bis zum ersten Deploy. Er läuft bewusst nicht in `npm run verify`
mit, damit ein Spiel während der Entwicklung im Vorlagenzustand grün bleiben
darf.
...
| --- | --- | --- |
| Starter-Template | 2.2.0 | Für neue Repositories vollständig verwenden; bestehende Spiele nicht blind überschreiben. |
