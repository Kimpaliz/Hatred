/* [Aufgabe: Prüfwesen] Das Licht als gebündelter Pixelpuffer — die
   ...
   `tests/pruefe-bild.mjs` misst das Licht seit jeher an den
   **einzelnen Zeichenaufrufen**: Nur dort sieht man einen halben
   ...
   `tests/buehne-browser.mjs` (`macheBildflaeche` und `bildpunkt` —
   das Blatt, das den Puffer mitschreibt), `tests/pruefe-bild.mjs`
   (misst dieselbe Funktion über den Rechteckweg), `spiel/gitter.mjs`
   (`macheKarte`), `tests/helfer.mjs` (Behauptungen und Abschluss)
