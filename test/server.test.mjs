...
test("liefert Health, Version, Repository-Medien und echte 404 ohne HTML-Fallback", async (context) => {
  ...
  // ein Vollbild kostet dort mehr Ladezeit als der gesamte restliche Katalog.
  for (const [name, budget] of Object.entries({ cover: 300 * 1024, hero: 400 * 1024, icon: 80 * 1024 })) {
    ...
    assert.ok(
      Number(head.headers.get("content-length")) <= budget,
      `${pathname} muss im Budget von ${Math.round(budget / 1024)} KB bleiben`
    );
    assert.notEqual(
